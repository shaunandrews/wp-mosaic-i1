...
import './ButtonWordPress.css';

interface ButtonWordPressProps {
  isToggled?: boolean;
  hasSidebarOpen?: boolean;
  onClick?: () => void;
}

export const ButtonWordPress = ({
  isToggled = false,
  hasSidebarOpen = false,
  onClick,
}: ButtonWordPressProps) => {
  return (
    <Button
      className={`button-wordpress ${hasSidebarOpen ? 'has-sidebar-open' : ''}`}
      isToggled={isToggled}
      align="center"
      onClick={onClick}
    >
